...
**네트워크 격리 규칙:**
- `proxy-net` (internal): api-proxy ↔ OpenClaw 통신. api-proxy가 Gemini API로 나감.
- `host-net`: OpenClaw 호스트 포트 바인딩 전용. api-proxy는 이 네트워크에 없음.
- `frontend` (internal, mem0 전용): OpenClaw ↔ Mem0 통신. 외부 접근 불가.
- `backend` (internal, mem0 전용): Mem0 ↔ Qdrant 통신. 외부 접근 불가.
