...
`proxyMode: "none"` (통합 프록시) 프로젝트에서 직접 import할 수 있는 TypeScript 네이티브 보안 모듈. Python api-proxy 컨테이너와 동일한 보안 가드를 구현합니다.

### 파일 구조

```
src/sdk/
├── index.ts                    # 배럴 export (@permissionlabs/claw-farm/security)
├── types.ts                    # 공유 인터페이스
│                                 Finding, PiiPattern, PiiPatternGroup, SecretPatternGroup,
│                                 LlmProvider, ProxyContext, ProxyResponse, RequestMiddleware
├── pii-redactor.ts             # PII 감지 + 편집
│                                 redactPii()          — 독립 함수
│                                 redactRequestBody()  — JSON 바디 워커
│                                 piiRedactor()        — 미들웨어 팩토리
├── secret-scanner.ts           # 시크릿 감지 + 제거
│                                 scanSecrets()        — 독립 함수
│                                 scanResponseBody()   — JSON/텍스트 바디 워커
│                                 secretScanner()      — 미들웨어 팩토리
├── audit-logger.ts             # JSONL 감사 로깅
│                                 createAuditLogger()  — 독립 팩토리
│                                 auditLogger()        — 미들웨어 팩토리
├── llm-proxy.ts                # 파이프라인 엔진
│                                 createLlmProxy()     — 미들웨어 조합 + 업스트림 fetch
├── patterns/                   # PII 패턴 그룹 (확장 가능한 레지스트리)
│   ├── index.ts                # defaultPatterns = korean + us + financial + universal
│   ├── korean.ts               # 8개 패턴: 주민번호, 전화번호 변형, 유선, 사업자등록, 여권, 운전면허
│   ├── us.ts                   # 2개 패턴: SSN, 전화번호
│   └── financial.ts            # 2개 패턴: 신용카드, 포맷된 카드번호
└── providers/                  # LLM 프로바이더 팩토리 (확장 가능)
    ├── index.ts                # 빌트인 export
    ├── types.ts                # LlmProvider 인터페이스
    ├── gemini.ts               # Gemini (disableThinking 옵션)
    ├── anthropic.ts            # Anthropic (anthropic-version 헤더 자동 추가)
    ├── openrouter.ts           # OpenRouter (Bearer 인증, referer/title)
    └── openai-compat.ts        # OpenAI 호환 (커스텀 baseUrl)
```

### 미들웨어 파이프라인 (어니언 모델)

```
요청 → [piiRedactor] → [커스텀미들웨어] → [secretScanner] → [auditLogger] → 업스트림 fetch
                                                                                    │
응답 ← [piiRedactor] ← [커스텀미들웨어] ← [secretScanner] ← [auditLogger] ← ──────┘
```

각 미들웨어는 `(ctx, next)`를 받습니다. `next()`를 호출하면 다음 미들웨어로 제어가 넘어갑니다. `next()`가 resolve된 후 응답을 검사/수정할 수 있습니다 (어니언 모델).

빌트인 보안 가드(경로 순회 방지, 프리픽스 검증, 콘텐츠 크기 제한, 헤더/쿼리 필터링)는 엔진 자체에서 적용되므로 실수로 제거할 수 없습니다.

### Python api-proxy와의 관계

| 배포 방식 | 프록시 메커니즘 | 사용 시기 |
|-----------|----------------|----------|
| `proxyMode: "per-instance"` / `"shared"` | Python api-proxy 컨테이너 | 독립 배포, 간단한 설정 |
| `proxyMode: "none"` | SDK를 TS 서버에 import | 커스텀 서버가 있는 서비스 지향 프로젝트 |

둘 다 동일한 보안 가드(PII 편집, 시크릿 스캐닝, 감사 로깅, 경로 검증)를 구현합니다. SDK 버전은 미들웨어를 통해 조합 가능하고 커스텀 패턴/프로바이더로 확장 가능합니다.
