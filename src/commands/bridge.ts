...
import { resolveExternalNetwork } from "../lib/api.ts";
...
const INSTANCE_OPERATIONS = new Set([
  ...
  "instance.sidecar.attach",
  "instance.sidecar.detach",
  "agent.create",
  "agent.updateConfig",
...
// ─── #171 Slice 2: Sidecar attach/detach control-plane operations ────────────

/**
 * Attach a per-instance weixin sidecar to an existing instance.
 *
 * Order: write pending spec → provision token → materialize compose → recreate target → health check → commit spec
 * Failure: revoke token, remove spec, restore old compose state
 */
async function bridgeInstanceSidecarAttach(payload: Record<string, unknown>): Promise<BridgeSuccess | BridgeFailure> {
  const { project, userId } = parseRuntimeInstanceKey(payload);
  validateBridgeName(userId, "user ID");
  const context = await requireManagedInstance("instance.sidecar.attach", project, userId);
  if ("ok" in context) return context;

  const managedInstanceId = asString(payload.managedInstanceId);
  const clawBayApiUrl = asString(payload.clawBayApiUrl);
  const clawBayAdminToken = asString(payload.clawBayAdminToken);
  const sidecarCode = asString(payload.sidecarCode) ?? "weixin-auth-sidecar";

  // Fail-closed: require all provisioning inputs
  ...
  if (missing.length > 0) {
    return bridgeFailure({
      action: "instance.sidecar.attach",
      message: `Missing required inputs: ${missing.join(", ")}`,
      errorCode: "invalid-payload",
    });
  }

  const projectName = context.resolved.name;
  const projectDir = context.resolved.entry.path;
  const instDir = join(projectDir, "instances", userId);
  const composeProject = `${projectName}-${userId}`;

  // Check if spec already exists and is enabled (idempotent)
  const existingSpec = await readSidecarSpec(instDir).catch(() => null);
  if (existingSpec?.enabled) {
    return bridgeSuccess({
      action: "instance.sidecar.attach",
      message: `Sidecar already attached for "${userId}"`,
      project: projectName,
      userId,
      runtimeState: "running",
      metadata: {},
  ...
  const spec: SidecarSpec = {
    ...
    envFile: ".env.weixin",
    port: 8787,
    externalNetwork: resolveExternalNetwork(),
    composeProject,
    updatedAt: new Date().toISOString(),
  };
  await writeSidecarSpec(instDir, spec);

  // Step 2: Provision token via ClawBay API
  ...
  try {
    const provisionResponse = await fetch(
      `${clawBayApiUrl!.replace(/\/$/, "")}/api/internal/weixin-binding-provision`,
      {
        method: "POST",
        headers: {
    ...
    if (!provisionResponse.ok) {
      ...
      throw new Error(`Token provision failed: ${provisionResponse.status} ${JSON.stringify(errorBody).slice(0, 200)}`);
    }
  } catch (error) {
    // Rollback: remove spec, revoke token (best-effort)
    ...
    return bridgeFailure({
      action: "instance.sidecar.attach",
      message: `Sidecar attach failed: ${error instanceof Error ? error.message : String(error)}`,
      errorCode: "runtime-command-failed",
    });
  }

  // Step 3: Recreate compose with sidecar (upInstance reads spec)
  try {
    ...
    // Rollback: revoke token, remove spec
    try {
      await fetch(`${clawBayApiUrl!.replace(/\/$/, "")}/api/internal/weixin-binding-provision/revoke`, {
        method: "POST",
        headers: {
    ...
    return bridgeFailure({
      action: "instance.sidecar.attach",
      message: `Sidecar compose recreation failed: ${error instanceof Error ? error.message : String(error)}`,
      errorCode: "runtime-command-failed",
    });
  }
  ...
  return bridgeSuccess({
    action: "instance.sidecar.attach",
    message: `Sidecar attached for "${userId}"`,
    project: projectName,
    userId,
    runtimeState: "running",
    metadata: {},
...
/**
 * Detach a per-instance weixin sidecar from an instance.
 *
 * Order: write disabled spec (blocks lifecycle) → teardown containers (--remove-orphans) → revoke token → commit (remove spec)
 * Failure: report degraded, do not pretend disabled
 */
async function bridgeInstanceSidecarDetach(payload: Record<string, unknown>): Promise<BridgeSuccess | BridgeFailure> {
  const { project, userId } = parseRuntimeInstanceKey(payload);
  validateBridgeName(userId, "user ID");
  const context = await requireManagedInstance("instance.sidecar.detach", project, userId);
  if ("ok" in context) return context;
  ...
  const spec = await readSidecarSpec(instDir).catch(() => null);
  if (!spec?.enabled) {
    return bridgeSuccess({
      action: "instance.sidecar.detach",
      message: `Sidecar not attached for "${userId}"`,
      project: projectName,
      userId,
      runtimeState: "running",
      metadata: {},
  ...
  // Step 1: Teardown sidecar container with --remove-orphans
  try {
    // Down the entire compose, then up without sidecar
    await downInstance(projectName, userId, { quiet: true });
  } catch (error) {
    // Continue with token revoke even if teardown partially failed
    // Report degraded in the response
  }
  ...
  if (managedInstanceId && clawBayApiUrl && clawBayAdminToken) {
    try {
      ...
      // Best effort — token may expire naturally
    }
  }

  // Step 3: Remove spec (commit detach)
  await removeSidecarSpec(instDir);

  // Step 4: Recreate compose without sidecar and start
  try {
    await upInstance(projectName, userId, { quiet: true });
  } catch (error) {
    return bridgeFailure({
      action: "instance.sidecar.detach",
      message: `Sidecar detached but runtime restart failed (degraded): ${error instanceof Error ? error.message : String(error)}`,
      errorCode: "runtime-command-failed",
    });
  }
  ...
  return bridgeSuccess({
    action: "instance.sidecar.detach",
    message: `Sidecar detached for "${userId}"`,
    project: projectName,
    userId,
    runtimeState: "running",
    metadata: {},
...
async function dispatch(operation: string, payload: Record<string, unknown>): Promise<BridgeSuccess | BridgeFailure> {
  ...
  try {
    switch (operation) {
      ...
      case "instance.sidecar.attach":
        return await bridgeInstanceSidecarAttach(payload);
      case "instance.sidecar.detach":
        return await bridgeInstanceSidecarDetach(payload);
      case "agent.create":
